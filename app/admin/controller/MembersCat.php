...
class MembersCat extends Base
{
    public function del(){
        ...
        // $list =  \app\common\model\mysql\MembersCat::order('paixu desc')->select();
        ...
        if ($request->isAjax()) {
            ...
            $id = $request->param("id");
            $newstype =  \app\common\model\mysql\MembersCat::where("`id`='$id'")->find();

            if ($newstype['id']) {

                $uptype =  \app\common\model\mysql\MembersCat::where("`pid`='$id'")->find();
                $r = array();
                if ($uptype['id']) {
                    $r['status'] = 0;
                    ...
                    $res =  \app\common\model\mysql\MembersCat::where("`id`='$id'")->delete(); // 删除id为5的用户数据
    ...
    public function index()
    {
        ...
        return View::fetch('', ["list" => \app\common\lib\Tree::tree($MembersCats)]);
    }
    public function add()
    {
        $request = $this->request;
        ...
        if ($request->isPost()) {
            ...
            if ($Varr['name']) {
                ...
                $brand = \app\common\model\mysql\MembersCat::where("`name`='$name'")->find();
                ...
                if (!$id) {
                    ...
                    $res = \app\common\model\mysql\MembersCat::create($Varr);
        ...
        return View::fetch('', [
            "catlist" =>  \app\common\lib\Tree::tree($MembersCats),
        ]);
        }
    }
    ...
    public function edit()
    {
        ...
        if (!$id) {
            ...
            if ($request->isPost()) {
                ...
                if ($name) {
                    ...
                    if (!$brand['id'] or $id == $brand['id']) {
                        ...
                        $res = \app\common\model\mysql\MembersCat::where("`id`=$id")->update($Varr);
            ...
            try {
                $find = (new MembersCatBus())->getById($id);
            } catch (\Exception $e) {
                $find = [];
                $this->error("分类id的内容不存在！", url('index'), 3);
            }
