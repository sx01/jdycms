...
is_dir($save_path) OR mkdir($save_path, 0755, true);   // 如果文件夹不存在，将以递归方式创建该文件夹
//文件保存目录URL
...
if (empty($_FILES) === false) {
    ...
    /* 水印配置开始 */
    $water_mark = 0;//1为加水印, 其它为不加
    $water_pos = 9;//水印位置，10种状态【0为随机，1为顶端居左，2为顶端居中，3为顶端居右；4为中部居左，5为中部居中，6为中部居右；7为底端居左，8为底端居中，9为底端居】
    $water_img = $_SERVER['DOCUMENT_ROOT'].'/static/image/common/watermark.png';//水印图片,默认填写空,请将图片上传至网站根目录的upfiles下,例: water.gif
    $water_alpha = 100;//水印透明度
